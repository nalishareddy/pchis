...
const NursePatientDetail = () => {
  ...
  const fetchPatient = async () => {
    try {
      const res = await api.get(`/nurse/patient/${id}`);
      setData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/nurse/patient/${id}/vitals`, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <button onClick={() => navigate('/nurse')} className="text-primary-600 text-sm font-medium flex items-center gap-1 mb-3 hover:underline">
          ← Back to Dashboard
        </button>
